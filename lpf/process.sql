...
--将电话号码与酒店对应起来
use tmp_htlbidb;
drop table if exists tmp_allcall_finally_htl;
create table tmp_allcall_finally_htl as
select 
	a.*, b.hotel
from 
tmp_htlbidb.tmp_allcall_finally as a
join
(
  	select distinct hotel, telephone 
  	from Dim_HtlDB.DimhtlHotel 
  	where telephone <> '0000000' and telephone <> '00000000' and telephone <> '000000000'
  	and telephone <> '0000000000' and  telephone <> '#'
)b  
on regexp_replace(a.dnis_no, '-|\\+|\\s|#', '') = regexp_replace(b.telephone, '-|\\+|\\s|#', '')
