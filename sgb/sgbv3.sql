...
select
	...
	d.isholdroom, d.holdroomtype, d.freesale, d.submitfrom, d.orderdate, d.remarktype,
    d.ordertype, d.iscorp, d.cost, d.ave_price, d.cii_amount, d.cii_amount_valid, d.ord_persons,
    d.ismaskedorder, d.frompackageorder, d.ord_roomstatus, d.ord_days, d.ord_roomnum, d.recommendlevel,
	(case when datediff(d.arrival, d.orderdate) = 0 then 1 else 0 end) as istodayord, d.isdelay,
	(unix_timestamp(d.arrival)-unix_timestamp(d.orderdate))/60 as arr2ord_time,--统计入住日与订单日相差的时间（单位min）
	d.cityid, d.isonline,
...
select 
	b.*, a.processtime,
	hour(a.processtime) as process_hour, pmod(datediff(a.processtime, '1920-01-01') - 3, 7) as process_week, -- outcall分配日是星期几
    hour(b.orderdate) as ord_hour, pmod(datediff(b.orderdate, '1920-01-01') - 3, 7) as ord_week, -- 下单日是星期几
	(unix_timestamp(b.confirmdate)-unix_timestamp(a.processtime))/60 as duration, -- 回复时间间隔（单位min）
	(unix_timestamp(a.processtime)-unix_timestamp(b.orderdate))/60 as process2ord_time, -- outcall分配时间到下单时间点的时间差（单位min）
    (unix_timestamp(b.arrival)-unix_timestamp(a.processtime))/60 as arr2process_time, -- arrival时间点到outcall分配时间点的时间差（单位min）
...
==================================================================================================================
use tmp_htlbidb;
drop table if exists tmp_sgb_htl_process_full_7_geo;
create table tmp_sgb_htl_process_full_7_geo as
select
    a.*, b.nums
from
tmp_htlbidb.tmp_sgb_htl_process_full_7 as a
join
(
    select
        hotel, processtime_1, count(*) as nums
    from tmp_htlbidb.tmp_sgb_htl_process_full_7
    group by hotel, processtime_1
)b
on a.hotel = b.hotel and a.processtime_1 = b.processtime_1
...
select 
    ...
    count(*) as duration_htl_process_hour_7_numsord,                        --过去一个星期子酒店分小时订单总数
    count(*)/7 as duration_htl_process_hour_7_preord,                       --过去一个星期子酒店分小时订单平均数
    exp(sum(log(power(duration, 1.0/nums)))) as duration_htl_process_hour_7_geomean
from tmp_htlbidb.tmp_sgb_htl_process_full_7_geo where process_hour = hour(processtime_1)
group by hotel, processtime_1
...
select
    hotel, processtime_1 as processtime,
    avg(duration) as duration_htl_process_week_7_pre,                       --过去一个星期子酒店订单回复时长平均值
    stddev(duration) as duration_htl_process_week_7_stddev,                 --过去一个星期子酒店订单回复时长标准差
    percentile_approx(duration, 0.75) as duration_htl_process_week_7_per75, --过去一个星期子酒店订单回复时长75分位
    percentile_approx(duration, 0.50) as duration_htl_process_week_7_per50, --过去一个星期子酒店订单回复时长中位数
    percentile_approx(duration, 0.25) as duration_htl_process_week_7_per25,  --过去一个星期子酒店订单回复时长25分位
    count(*) as duration_htl_process_week_7_numsord,                        --过去一个星期子酒店订单总数
    count(*)/7 as duration_htl_process_week_7_preord,                       --过去一个星期子酒店订单平均数
    exp(sum(log(power(duration, 1.0/nums)))) as duration_htl_process_week_7_geomean
from tmp_htlbidb.tmp_sgb_htl_process_full_7_geo
group by hotel, processtime_1
...
==================================================================================================================
use tmp_htlbidb;
drop table if exists tmp_sgb_htl_ord_full_7_geo;
create table tmp_sgb_htl_ord_full_7_geo as
select
    a.*, b.nums
from
tmp_htlbidb.tmp_sgb_htl_ord_full_7 as a
join
(
    select
...
select 
    ...
    count(*)/7 as duration_htl_ord_hour_7_preord,                           --过去一个星期子酒店分小时订单平均数
    exp(sum(log(power(duration, 1.0/nums)))) as duration_htl_ord_hour_7_geomean
from tmp_htlbidb.tmp_sgb_htl_ord_full_7_geo where ord_hour = hour(orderdate_1)
group by hotel, orderdate_1
...
select
    ...
    count(*) as duration_htl_ord_week_7_numsord,                            --过去一个星期子酒店订单总数
    count(*)/7 as duration_htl_ord_week_7_preord,                           --过去一个星期子酒店订单平均数
    exp(sum(log(power(duration, 1.0/nums)))) as duration_htl_ord_week_7_geomean
from tmp_htlbidb.tmp_sgb_htl_ord_full_7_geo
group by hotel, orderdate_1
...
==============================================================================================================
use tmp_htlbidb;
drop table if exists tmp_sgb_htl_process_full_30_geo;
create table tmp_sgb_htl_process_full_30_geo as
select
    a.*, b.nums
from
tmp_htlbidb.tmp_sgb_htl_process_full_30 as a
join
(
    select
        hotel, processtime_1, count(*) as nums
...
select 
    ...
    count(*) as duration_htl_process_hour_30_numsord,                        --过去一个月子酒店分小时订单总数
    count(*)/7 as duration_htl_process_hour_30_preord,                       --过去一个月子酒店分小时订单平均数
    exp(sum(log(power(duration, 1.0/nums)))) as duration_htl_process_hour_30_geomean
from tmp_htlbidb.tmp_sgb_htl_process_full_30_geo where process_hour = hour(processtime_1)
group by hotel, processtime_1
...
select
    ...
    count(*) as duration_htl_process_month_30_numsord,                              --过去一个月子酒店分小时订单总数
    count(*)/7 as duration_htl_process_month_30_preord,                             --过去一个月子酒店分小时订单平均数
    exp(sum(log(power(duration, 1.0/nums)))) as duration_htl_process_month_30_geomean
from tmp_htlbidb.tmp_sgb_htl_process_full_30_geo
group by hotel, processtime_1
...
==============================================================================================================
use tmp_htlbidb;
drop table if exists tmp_sgb_htl_ord_full_30_geo;
create table tmp_sgb_htl_ord_full_30_geo as
select
    a.*, b.nums
from
tmp_htlbidb.tmp_sgb_htl_ord_full_30 as a
join
(
    select
        hotel, orderdate_1, count(*) as nums
...
select 
    ...
    count(*)/7 as duration_htl_ord_hour_30_preord,                          --过去一个月子酒店分小时订单平均数
    exp(sum(log(power(duration, 1.0/nums)))) as duration_htl_ord_hour_30_geomean
from tmp_htlbidb.tmp_sgb_htl_ord_full_30_geo where ord_hour = hour(orderdate_1)
group by hotel, orderdate_1
...
select
    ...
    count(*) as duration_htl_ord_month_30_numsord,                              --过去一个月子酒店分小时订单总数
    count(*)/7 as duration_htl_ord_month_30_preord,                             --过去一个月子酒店分小时订单平均数
    exp(sum(log(power(duration, 1.0/nums)))) as duration_htl_ord_month_30_geomean
from tmp_htlbidb.tmp_sgb_htl_ord_full_30_geo
group by hotel, orderdate_1
...
select 
	...
	h.duration_htl_ord_hour_7_numsord, h.duration_htl_ord_hour_7_preord, 
	h.duration_htl_ord_hour_7_geomean
from
(
	select 
		...
		f.duration_htl_ord_week_7_numsord, f.duration_htl_ord_week_7_preord, 
		f.duration_htl_ord_week_7_geomean
	from
	(
		select 
			...
			d.duration_htl_process_hour_7_numsord, d.duration_htl_process_hour_7_preord, 
			d.duration_htl_process_hour_7_geomean
		from
		(
			select 
				...
				b.duration_htl_process_week_7_numsord, b.duration_htl_process_week_7_preord, 
				b.duration_htl_process_week_7_geomean
			from
...
select 
	...
	h.duration_htl_ord_hour_30_numsord, h.duration_htl_ord_hour_30_preord, 
	h.duration_htl_ord_hour_30_geomean
from
(
	select 
		...
		f.duration_htl_ord_month_30_numsord, f.duration_htl_ord_month_30_preord, 
		f.duration_htl_ord_month_30_geomean
	from
	(
		select 
			...
			d.duration_htl_process_hour_30_numsord, d.duration_htl_process_hour_30_preord, 
			d.duration_htl_process_hour_30_geomean
		from
		(
			select 
				...
				b.duration_htl_process_month_30_numsord, b.duration_htl_process_month_30_preord, 
				b.duration_htl_process_month_30_geomean
			from
				tmp_htlbidb.tmp_sgb_duration_positive as a
			join
...
select 
	...
	b.duration_htl_ord_week_7_pre,
	b.duration_htl_ord_week_7_stddev, b.duration_htl_ord_week_7_per75,
	b.duration_htl_ord_week_7_per50, b.duration_htl_ord_week_7_per25,
	b.duration_htl_ord_week_7_numsord, b.duration_htl_ord_week_7_preord, 
	b.duration_htl_ord_week_7_geomean,
	b.duration_htl_process_hour_7_pre,
	b.duration_htl_process_hour_7_stddev, b.duration_htl_process_hour_7_per75,
	b.duration_htl_process_hour_7_per50, b.duration_htl_process_hour_7_per25,
	b.duration_htl_process_hour_7_numsord, b.duration_htl_process_hour_7_preord, 
	b.duration_htl_process_hour_7_geomean,
	b.duration_htl_process_week_7_pre,
	b.duration_htl_process_week_7_stddev, b.duration_htl_process_week_7_per75,
	b.duration_htl_process_week_7_per50, b.duration_htl_process_week_7_per25,
	b.duration_htl_process_week_7_numsord, b.duration_htl_process_week_7_preord, 
	b.duration_htl_process_week_7_geomean
from 
tmp_htlbidb.tmp_sgb_htl_f30 as a
left join
tmp_htlbidb.tmp_sgb_htl_f7 as b
on a.orderid = b.orderid


hive -e "select * from tmp_htlbidb.tmp_sgb_htl_fall" > tmp_sgb_htl_fall.csv

hive -e "select * from tmp_htlbidb.tmp_sgb_htl_fall where orderdate >= '2018-02-01' and orderdate < '2018-04-01'" > tmp_sgb_htl_fall1.csv
